...
namespace Tibr.Application.Services.PlanServices
{
    ...
    {
        Task<Result<PlanDto>> CreateFromGoalAsync(long userId, GoalParseResult goal, decimal priceAtCreation, decimal? silverPriceAtCreation);
        Task<Result<PlanDto>> GetActivePlanAsync(long userId);
        Task<Result<ReevaluatePlanResultDto>> ReevaluateAsync(long userId);
